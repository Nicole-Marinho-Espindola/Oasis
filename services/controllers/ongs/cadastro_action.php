    ...
    if (!empty($_POST) && !isset($_SESSION['cadastro_realizado'])) {
        ...
        try {
            ...
            if ($stmt_verificar->rowCount() > 0) {
                header("Location: ../../views/forms/ongs/cadastro.php?email_repetido=true");
            } else {
                $hashDaSenha = password_hash($senha, PASSWORD_DEFAULT);

                $stmt = $conn->prepare("INSERT INTO tb_ong(nm_ong, nm_razao, cd_cnpj, ds_endereco_ong, ds_missao, cd_celular_ong, ds_email, cd_senha)
                                    VALUES (:nome, :razao, :cnpj, :endereco, :missao, :celular, :email, :senha)");
                ...
                //adicionar aqui embaixo o redirecionamento para confirmação de email, etc.
                header("Location: ../../../views/forms/ongs/confirmarEmail.php");
                exit();
            }
