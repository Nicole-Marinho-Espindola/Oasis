<?php
    ...
    if (!empty($_POST)) {
        ...
        $id = filter_input(INPUT_POST, 'cd_voluntario', FILTER_DEFAULT);

        try {

            $stmt = $conn->prepare("SELECT cd_voluntario, nm_voluntario, ds_email FROM tb_voluntario WHERE cd_voluntario = :id LIMIT 1");
            ...
            if ($stmt->rowCount() != 0) {
                ...
                $stmtToken->bindParam(':tipo_token', $tipoToken, PDO::PARAM_INT);
                $stmtToken->bindParam(':voluntario', $row['cd_voluntario'], PDO::PARAM_INT);
